...
> AI 说明：项目内置的 Jev 是本地 TypeScript 日志规则诊断引擎，不是开源大模型，也不包含或下载模型权重。远程模式仅在用户自行配置兼容端点、密钥和模型标识后启用。
...
- **事件与告警通知：** 支持通用 Webhook（深度兼容钉钉机器人、企业微信应用机器人、飞书）、Bark（iOS 极简推送）、Server酱（微信推送）、PushPlus（推送加）、Telegram、Home Assistant、ntfy、Discord 与邮件通知渠道，每个渠道可独立配置告警级别过滤。内置防抖动、重复抑制、静音时段及未恢复告警周期提醒，防止告警风暴。**时间线 (Timeline)** 将审计日志、运行事件与指标异常波动合并展示（如“22:00 修改了配置，22:05 容器异常崩溃”）。
...
- **Docker 全生命周期管理：** 容器、堆栈 (Compose)、镜像、数据卷、网络与清理维护。
  ...
  - **动态资源配额调整：** 支持无需停机或重启容器，动态热调整运行中容器的 CPU 核心配额与内存限制（基于 Docker Update API）。
  - **Docker 镜像加速源管理：** 内置网易、中科大、南京大学、交大等主流加速源，支持界面测速、延迟检测与一键写入 `/etc/docker/daemon.json`。
  - **容器日志安全截断：** 支持一键安全清空超大容器底层日志文件（释放磁盘空间，同时不破坏容器的标准输出 stdout 句柄）。
  - **Web 终端触控优化：** 集成 xterm.js，专为移动端与触控设备提供虚拟功能键浮窗（Esc、Tab、Ctrl+C、Ctrl+L、方向键等），支持字号快捷缩放。
  - 实时 ANSI 彩色日志、容器内 Web 终端、容器文件浏览器与编辑器、单容器资源监控图表。
...
本项目在原作者 `xiaoxinkeji` 的架构、功能设计与早期实现基础上持续演进。后续维护重点包括中文运维体验、可靠性、安全边界、Docker 生命周期管理以及 AI 辅助运维能力。

感谢原作者为项目打下的基础，也感谢 Next.js、React、TypeScript、Docker、Caddy、SQLite 及相关开源项目的维护者与贡献者。新增改动尽量保持原有架构约束，并通过测试、类型检查、Lint、OpenAPI 校验和生产构建进行验证。

如果你基于本项目继续开发，请保留原作者及上游项目的版权、许可证和贡献说明，并在提交功能改动时补充清晰的变更记录。
